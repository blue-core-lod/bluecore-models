...
def test_work_hub_link_is_saved_from_hasExpression(pg_session):
    ...
    LC writes bf:expressionOf from the Work, but BIBFRAME declares
    bf:hasExpression its owl:inverseOf, so a source that writes the Hub end
    instead is writing valid BIBFRAME and must still end up linked.
    """
    work_uri = f"https://bcld.info/works/{uuid.uuid4()}"
    hub_uri = f"https://bcld.info/hubs/{uuid.uuid4()}"
